...
export function Credencial({data, OnChangeAvatar,onExpamdQRCode}: 
    Props){
        const {height} = useWindowDimensions()
    return(
        <MotiView className=" w-full self-stretch items-center" from = {
            { opacity: 0, translateY: -height, rotateZ: "50deg", rotateY: "30deg", rotateX: "30deg"}} 
            ...
            <View className="bg-black/20 self-stretch items-center pb-6 border border-white 
            10 mx-3 rounded-2xl -mt-5"> 

            <ImageBackground source={require("@/assets/ticket/header.png")} 
            className="px-6 py-8 h-40 items-center self-stretch border-b border-white 
            overflow-hidden">
            ...
            <TouchableOpacity activeOpacity={0.7} className="mt-6" onPress={onExpamdQRCode}>
            <Text className="font-body text-orange-500 text-sm">Amplicar QrCode</Text>
            </TouchableOpacity>
            </View>
        </MotiView>
    )
